...
自販機や券売機などで利用されており、見た目は図～のような感じです。
...
コインの真贋判定部分だけ行う部品で、見た目としては図～のような感じです。

100円限定など1金種のみ対応、メダル用など自分で学習させるタイプがあります。
...
自販機や券売機などで利用されるような機種の見た目は図～のような感じです。
...
制御方式にも違いがあり、図～のはパラレル制御になっています。
詳細は調べていないので不明ですが、紙幣を投入すると特定ピンがONになったりするらしいです。
自販機で使われるタイプは図～で、これはコインメックと同じJVMA方式です。
今回はこのタイプのビルバリを入手し制御してみます。

=== JVMA方式
コインメックとビルバリの制御はJVMA方式で行われることがわかりました。
ここではJVMA方式@<fn>{vccs}について調べていきます。
//footnote[vccs][VCCSとも呼ばれるようです。シリアル通信のため「シリアル伝送方式」とも、また後述の特許を読むと「ベンダーコントロールシステム」「VCSバス」と書かれていることもありました。]
...
それでも特許情報を調べることである程度理解できました。
また、コインメックやビルバリをPC接続し制御する基板がいくつか販売されています。
法人向けのため個人で購入することはまず無理だと思いますが、取扱説明書だけは見ることができました。
JVMA方式の制御コマンドそのものは記載されていませんが、基板側のコマンドは載っておりおぼろげながらJVMA方式のコマンドも浮かんできます。
さらに勢い余って券売機@<fn>{jihanki}@<fn>{jihanki2}を購入してしまいました。
実際に硬貨を投入した際の信号をロジアナで追いかけると、新たな発見もありました。
（でも実は特許に書いてあったものを見落としていただけだったりして必ずしも券売機まで買う必要はあったか、特許+取扱説明書に加えて試行錯誤すれば目的は達成出来たのではないかと思っています。）
//footnote[jihanki][卓上券売機です。本当は自販機が欲しかったのですが部屋には置けない、近所のガレージを借りる気でいたのですが満室で借りられない。券売機にもコインメックがあるのでは？卓上券売機なら部屋に置けるのでは？ということで購入しました。]
//footnote[jihanki2][部屋に自販機を置く場合はこちらが参考になると思います @<href>{https://twitter.com/uni_mafumafu/status/1444240003610021894} @<href>{https://www.youtube.com/watch?v=9c4DjWidCVU}]

==== 物理層・データリンク層
こうして得たJVMA方式の情報をまとめます。
最初は物理層・データリンク層です。

まず特許と気になった記述を引用します。

: 三洋電機株式会社．芝野正雅．自動販売機の制御装置．特開２０００－１７２９０８
...
: 富士電機株式会社．松藤宏．貨幣鑑別装置の通信監視装置．特開２００１－３４８２１
...
: ザコカコーラカンパニー．テイラーキツト・ダブリユー，サシロウスキウオルター，ラニアノエル．販売機デュアル母線構成．特表２００２－５１０１０３
...
: 富士電機リテイルシステムズ株式会社．松藤宏．自動販売機．特開2009-223677
 ...
 * 電源線がある
   ** 24V および 8V
 * 同期線がある
   ** 「ＳＹＮラインをロー（Ｌ）にしてから通信を始め、最初の１バイトの途中でハイ（Ｈ）に戻す。」（特開２０００－１７２９０８）
   ** 「コマンドやデータを出力する直前に数１００μｓのパルス幅で出力」（特開２００１－３４８２１）
...
多少曖昧な箇所はありますが、これだけ分かればあとは試行錯誤でどうにか出来そうです。

==== コマンド
そしてコマンドについてです。
が、これについては自信がありません。

Yahoo!知恵袋みたいなことを書いて申し訳ありませんが、説明書を探すか券売機買って確かめてください。
もし無い場合でも、コマンドが5bit+3bitなら何か分かりそうじゃないですか？
