...
//profile[lrks;twitter:__lrks__][ねっとりNetlink、精算機作るよ！]{
...
//subfig[生産した精算機]{
//image[fusion360][顧客が説明した案件][scale=0.4]
//image[s3][実装された運用][scale=0.4]
//}
...
自販機や券売機などで利用されるタイプの見た目は@<img>{g200}、@<img>{fevt451}、@<img>{nri}のような感じです。
ガチャガチャで硬貨を入れる部分も「コインメック」と呼ばれますが、自販機用と比べて機能が大きく異なるので割愛します。

//subfig[コインメック（画像は各社カタログより引用）]{
//image[g200][コンラックス CLX-G200シリーズ][scale=0.2]
//image[fevt451][富士電機 FEVT451][scale=0.2]
//image[nri][NRI Currenza C2][scale=0.2]
//}
...
通信方式として、国内では@<b>{JVMA方式}@<fn>{jvma}が採用されています。
海外ではMDB/ICP、ccTalk、DEXなどが使われているようですが、よく調べていないため割愛します。
MDBについては仕様が公開されているようで、もし日本円仕様のコインメックが手に入れば制御しやすいかも知れません。
...
自販機や券売機などで利用されるような機種の見た目は@<img>{bve21s4}や@<img>{m400}、@<img>{nb}のような感じです。
...
//image[bve21s4][富士電機 BVE21S4][scale=0.22]
...
自販機などで使われるタイプは@<img>{bve21s4}や@<img>{m400}で、これはコインメックと同じJVMA方式です。
...
さらに勢い余って@<img>{ticketvend}の食券券売機@<fn>{jihanki}を購入してしまいました。
本当は自販機が欲しかったのですが部屋には置けない@<fn>{jihanki2}、レンタルガレージを借りる気でいたのですが条件が合うところは満室で借りられない。
券売機にもコインメックがあるのでは？卓上券売機なら部屋に置けるのでは？ということで購入しました。
動作が不安定@<fn>{notworking}、サポートなしという筐体でも結構します。もっと他のことにお金使ったほうが良いんじゃないですかね。。。
とはいえ、実機があると検証もしやすかったことは事実です。
でもやっぱり必ずしも券売機まで買う必要はあったか、特許公報に加えて試行錯誤すれば目的は達成出来たのではと思う気持ちもあります。
//image[ticketvend][らぁめぇ しぐこう 開店][scale=0.5]
//footnote[jihanki][もともとラーメン屋（閉店済み）で使われていたようです。メニューと価格を基に食べログなどを見ると現役時代の姿が確認できます。]
//footnote[jihanki2][一応部屋に自販機を置く先行事例はあります。 事例1:@<href>{https://twitter.com/uni_mafumafu/status/1444240003610021894} 事例2:@<href>{https://www.youtube.com/watch?v=9c4DjWidCVU}]
//footnote[notworking][実際ビルバリはまともに使えず。また検証には影響しませんが印字はかなり薄い状態でした。]
...
そのため、今回は記載しません。
決してまだ調べていないわけではなく、調査が済んで制御はできています。
本当です。信じてください。
...
自分で降圧回路を組むとトラブルが起きそう、極力簡単にしたかったのですが仕方ありません。
...
//footnote[qk30][QK30-Uではなく、パスポート読み取りに対応したQK30-OP-U。より正確にはパスポート読み取り用アタッチメントを取り外した何かです。]
//footnote[copy][執筆時点ではまだ触っていません。もし当日このQK30が取り外されていたら…そういうことです。]
