...
自販機や券売機などで利用されており、見た目は下図のような感じです。
...
//image[bve21s4][富士電機 BVE21S4][scale=0.3]
//image[m400][コンラックス NBX(R)-M400シリーズ][scale=0.3]
//image[nb][コンラックス NB-4MB80シリーズ][scale=0.3]
//}
...
//footnote[vccs][VCCSとも呼ばれるようです。シリアル通信のため「シリアル伝送方式」とも、また後述の特許を読むと「ベンダーコントロールシステム」「VCSバス」と書かれていることもありました。あと海外メーカーの資料だと「JPSTD」とも。]
...
多少曖昧な箇所はありますが、これだけ分かればあとは試行錯誤でいけそうです。
あとは流れでコマンドも分かるんじゃないでしょうか。
...
挟むものは何でも良いのですが、ここではRaspberry Pi Picoを使うことにしました。
上位機器ともUSBシリアルで利用できますし、マルチコアになっているのも便利でした。
...
//footnote[copy][執筆時点でも触っていません。今回がコピー本なのも納得ですね。]

実はNAYAXのリーダー（@<img>{nayax}）をあわよくば使えるかな？モニタになにか表示したりQRコード読み取りさせたり？と思って買いましたが、
あわよくいかなかったので断念しました。
//image[nayax][NAYAX][scale=0.49]
...
本当はCNCを買って基板加工したいが集合住宅だと無理、レーザーカッターも買ってアクリルやMDF切りまくりたいですが排気がムリムリかたつむりです。

今回のケース？というかコインメックやビルバリ、Square Terminal、QK30を固定するフレームも3Dプリンタで出力しました。
パネルは作っていませんが、いずれアクリル板とかで覆いたいですね。
1枚だけの加工ならアクセアが一番安くて手軽と思っているのでそこに頼む予定です。
