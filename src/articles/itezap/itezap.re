...
こんにちは、itezaPと申します。
まだまだ予断を許さない状況下ではありますが、ひとまず無事（?）にC100を迎えられたこと大変めでたく思います。
...
見分けるポイントは、黒目や瞳のグラデーションといった瞳の描き方です。
このように「見たことあるけど名前が思い出せない」や「この描き方好きだから、似た人いないかな」という画像検索がしたいことがあります。

//image[comp_illust][判別しやすい目と判別しにくい目の比較]

異なる画像同士の類似度を計算する手法は数多く提案されています。
類似度は全体の色合いや物体の輪郭など、比較したい目的に応じて定義されます。
それらの特徴を比較するために、何らかの数値やベクトルである特徴量を計算します。
そして、画像同士の特徴量の距離を計算することで求めることができます。
画像比較による類似画像検索サービスとして、Google画像検索が挙げられます。
画像の直接比較によらない方法の一つとしては、タグ付けによる類似画像検索が挙げられます。
様々なサイトが行なっていますが、イラストだとPixivやニコニコ静画などが有名です。
タグ検索は、構図は全く似ていないが特定のキャライラストを検索したい、といった検索が可能となります。
ですがこれらのサービスでは、ある特定部分の描き方が類似するイラストを検索したい、という細かい検索はまだ不十分です。

今回は新たなイラスト類似検索の試みの前段階として、イラストの目に注目したイラストレーター識別器の試作を開発してみました。
...
参考として、ネットワークの概要を@<img>{network_summary}に示します。

//image[network_summary][ネットワークの概要][scale=0.8]

=== データセット

データセットはいくつかのイラストレーターを対象に、ゲーム公式サイトや検索によって得られたキャラクターイラストを使用しました。
画像データは事前に目周辺の部分のみを切り抜き、右目と左目を別画像としました。
また、目が閉じてあるものは除外しました。
...
分散は2乗ですので、平均予測値から0.03程度誤差があることを表しています。
...
過去の絵柄と誤認識した例を見ると、瞳の描き方が従来と異なることが見て取れます。
そのため別クラスとして誤認識されてしまったと考えられます。
...
ダンジョントラベラーズ2は2013年発売のため、描き方は近年の描き方（ラベル1）にそれなりに近いです。
正しく認識させるには、ToHeart2や1999年に発売されたこみっくパーティ時代の画像の充実が課題と考えられます。

//image[mistake_predict_3][年代を誤認識した画像の一例]
